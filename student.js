populateBranchSelect("studentBranch");

const loginSection = document.getElementById("studentLogin");
const appSection = document.getElementById("studentApp");
const loginForm = document.getElementById("studentLoginForm");

let currentStudentEmail = sessionStorage.getItem("yuvaCurrentStudent");

function showStudentApp() {
  loginSection.classList.add("hidden");
  appSection.classList.remove("hidden");

  document.getElementById("studentEmail").value = currentStudentEmail;
...
function loadStudentProfile() {
  ...
  const student = students.find(
    (item) => item.email === currentStudentEmail
  );
  ...
  document.getElementById("studentSkills").value =
    (student.currentSkills || []).join(", ");

  updateStudentRoles();

  if (student.targetRole) {
    document.getElementById("studentTargetRole").value =
      student.targetRole;
  }
}
...
function updateStudentRoles() {
  ...
  const roleSelect =
    document.getElementById("studentTargetRole");

  const industryData = getIndustryData().filter(
    (item) =>
      item.status === "Active" &&
      item.branch === branch
  );
...
document
  .getElementById("saveStudentProfile")
  .addEventListener(
    "click",
    saveStudentProfile
  );
...
function saveStudentProfile() {
  ...
  student.currentSkills = skillArray(
    document.getElementById("studentSkills").value
  );
...
document
  .getElementById("compareStudentSkills")
  .addEventListener(
    "click",
    compareStudent
  );
...
function compareStudent() {
  ...
  const currentSkills = skillArray(
    document.getElementById("studentSkills").value
  );
  ...
  result.innerHTML = `
    ...
    <h3>Additional / Not Required Skills</h3>
  ...
  student.industryCompany = industry.company;
